...
describe("Mongo Repository tests", () => {
    ...
    test("Should return paginated users when getPaginate success", async () => {
        const sut = makeSut();
    ...
    test("Should increment user with wrong way", async () => {
        ...
        const { insertedId } = await userCollection.insertOne(user);
        const userIncremented = sut.increment({ _id: insertedId }, { name: "New John Doe" });
        expect(userIncremented).toBeTruthy();

        await expect(userIncremented).rejects.toThrow(
    ...
    test("Should increment user with success", async () => {
        ...
        const { insertedId } = await userCollection.insertOne(user);
        const userIncremented = await sut.increment({ _id: insertedId }, { appointments: 1 });
        expect(userIncremented).toBeTruthy();
        expect(userIncremented).toEqual({ _id: insertedId, ...user, appointments: 1 });
    });
