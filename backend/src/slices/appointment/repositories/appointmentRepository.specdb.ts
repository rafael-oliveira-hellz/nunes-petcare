...
describe("Appointment Mongo Repository", () => {
    ...
    beforeAll(async () => {
        ...
        repository.add.mockResolvedValue(appointmentEntityMock);
        repository.getOne.mockResolvedValue(appointmentEntityMock);
        repository.update.mockResolvedValue(appointmentEntityMock);
        repository.getPaginate.mockResolvedValue(appointmentEntityPaginatedMock?.appointments);
        repository.getCount.mockResolvedValue(appointmentEntityPaginatedMock?.total);
    ...
    test("should call add of addAppointment with correct values", async () => {
        await testInstance.addAppointment(appointmentEntityMock);
        expect(repository.add).toHaveBeenCalledWith(appointmentEntityMock);
        expect(repository.add).toHaveBeenCalledTimes(1);
    });
    test("should return a new appointment created when addAppointment insert it", async () => {
        const result = await testInstance.addAppointment(appointmentEntityMock);
        expect(result).toEqual(appointmentEntityMock);
    });
    ...
    test("should rethrow if add of addAppointment throws", async () => {
        ...
        const result = testInstance.addAppointment(appointmentEntityMock);
    ...
    test("should rethrow if update of updateAppointment throws", async () => {
        ...
        const result = testInstance.updateAppointment(fakeQuery, appointmentEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should call update of updateAppointment with correct values", async () => {
        await testInstance.updateAppointment(fakeQuery, appointmentEntityMock);
        expect(repository.update).toHaveBeenCalledWith(fakeQuery?.fields, appointmentEntityMock);
        expect(repository.update).toHaveBeenCalledTimes(1);
    });
    test("should return a appointment updated when updateAppointment update it", async () => {
        const result = await testInstance.updateAppointment(fakeQuery, appointmentEntityMock);
        expect(result).toEqual(appointmentEntityMock);
    });
    test("should return a appointment updated when updateAppointment update it when i pass null", async () => {
        const result = await testInstance.updateAppointment(null as any, appointmentEntityMock);
        expect(result).toEqual(appointmentEntityMock);
    });
    ...
    test("should return a appointmentByPage when loadAppointmentByPage loaded it", async () => {
        ...
        expect(result).toEqual(appointmentEntityPaginatedMock);
    });
