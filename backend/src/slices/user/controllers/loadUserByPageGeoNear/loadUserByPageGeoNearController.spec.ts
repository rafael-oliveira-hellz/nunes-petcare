...
import { userEntityMock, userEntityPaginatedMock } from "@/slices/user/entities/UserEntity.spec";
...
describe("LoadUserByPageGeoNearController", () => {
    ...
    beforeAll(async () => {
        ...
        loadUserByPageGeoNear.mockResolvedValue(userEntityPaginatedMock);
        validation = mock();
    ...
    beforeEach(() => {
        fakeQueryParams = { _id: userEntityMock._id };
    ...
    test("should call loadUserByPageGeoNear with correct params", async () => {
        ...
        expect(result).toEqual(ok(userEntityPaginatedMock));
        expect(loadUserByPageGeoNear).toHaveBeenCalledWith({
            ...
            options: {
                ...
                userLoggedId: userEntityMock?._id,
            },
        });
