...
import { UserData, UserPaginatedData } from "@/slices/user/entities";
import {
    AddUserRepository,
        ...
        LoadUserByPageGeoNearRepository
{
    ...
    async loadUserByPageGeoNear(query: Query): Promise<UserPaginatedData | null> {
        ...
        const resultGeoNearPaginatedArray = (await this.repository.aggregate(queryBuilded)) ?? [];
    ...
    async loadUserByPage(query: Query): Promise<UserPaginatedData | null> {
