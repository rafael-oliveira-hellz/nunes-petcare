...
describe("Order Mongo Repository", () => {
    ...
    beforeAll(async () => {
        ...
        repository.add.mockResolvedValue(orderEntityMock);
        repository.getOne.mockResolvedValue(orderEntityMock);
        repository.update.mockResolvedValue(orderEntityMock);
        repository.getPaginate.mockResolvedValue(orderEntityPaginatedMock?.orders);
        repository.getCount.mockResolvedValue(orderEntityPaginatedMock?.total);
    ...
    test("should call add of addOrder with correct values", async () => {
        await testInstance.addOrder(orderEntityMock);
        expect(repository.add).toHaveBeenCalledWith(orderEntityMock);
        expect(repository.add).toHaveBeenCalledTimes(1);
    });
    test("should return a new order created when addOrder insert it", async () => {
        const result = await testInstance.addOrder(orderEntityMock);
    ...
    test("should rethrow if add of addOrder throws", async () => {
        ...
        const result = testInstance.addOrder(orderEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should rethrow if update of updateOrder throws", async () => {
        repository.update.mockRejectedValueOnce(new Error("Error"));
        const result = testInstance.updateOrder(fakeQuery, orderEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should call update of updateOrder with correct values", async () => {
        await testInstance.updateOrder(fakeQuery, orderEntityMock);
        expect(repository.update).toHaveBeenCalledWith(fakeQuery?.fields, orderEntityMock);
        expect(repository.update).toHaveBeenCalledTimes(1);
    });
    test("should return a order updated when updateOrder update it", async () => {
        const result = await testInstance.updateOrder(fakeQuery, orderEntityMock);
        expect(result).toEqual(orderEntityMock);
    });
    test("should return a order updated when updateOrder update it when i pass null", async () => {
        const result = await testInstance.updateOrder(null as any, orderEntityMock);
        expect(result).toEqual(orderEntityMock);
    });
    ...
    test("should return a orderByPage when loadOrderByPage loaded it", async () => {
        ...
        expect(result).toEqual(orderEntityPaginatedMock);
    });
