...
describe("Category Mongo Repository", () => {
    ...
    beforeAll(async () => {
        ...
        repository.add.mockResolvedValue(categoryEntityMock);
        repository.getOne.mockResolvedValue(categoryEntityMock);
        repository.update.mockResolvedValue(categoryEntityMock);
        repository.getPaginate.mockResolvedValue(categoryEntityPaginatedMock?.categories);
        repository.getCount.mockResolvedValue(categoryEntityPaginatedMock?.total);
    ...
    test("should call add of addCategory with correct values", async () => {
        await testInstance.addCategory(categoryEntityMock);
        expect(repository.add).toHaveBeenCalledWith(categoryEntityMock);
        expect(repository.add).toHaveBeenCalledTimes(1);
    });
    test("should return a new category created when addCategory insert it", async () => {
        const result = await testInstance.addCategory(categoryEntityMock);
    ...
    test("should rethrow if add of addCategory throws", async () => {
        ...
        const result = testInstance.addCategory(categoryEntityMock);
    ...
    test("should rethrow if update of updateCategory throws", async () => {
        ...
        const result = testInstance.updateCategory(fakeQuery, categoryEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should call update of updateCategory with correct values", async () => {
        await testInstance.updateCategory(fakeQuery, categoryEntityMock);
        expect(repository.update).toHaveBeenCalledWith(fakeQuery?.fields, categoryEntityMock);
        expect(repository.update).toHaveBeenCalledTimes(1);
    });
    test("should return a category updated when updateCategory update it", async () => {
        const result = await testInstance.updateCategory(fakeQuery, categoryEntityMock);
        expect(result).toEqual(categoryEntityMock);
    });
    test("should return a category updated when updateCategory update it when i pass null", async () => {
        const result = await testInstance.updateCategory(null as any, categoryEntityMock);
        expect(result).toEqual(categoryEntityMock);
    });
    ...
    test("should return a categoryByPage when loadCategoryByPage loaded it", async () => {
        ...
        expect(result).toEqual(categoryEntityPaginatedMock);
    });
    test("should return null when loadCategoryByPage returns null", async () => {
        ...
        const result = await testInstance.loadCategoryByPage(fakeQuery);
        expect(result).toEqual({ categories: null, total: 0 });
    });
