...
describe("Owner Mongo Repository", () => {
    ...
    beforeAll(async () => {
        ...
        repository.add.mockResolvedValue(ownerEntityMock);
        repository.getOne.mockResolvedValue(ownerEntityMock);
        repository.update.mockResolvedValue(ownerEntityMock);
        repository.getPaginate.mockResolvedValue(ownerEntityPaginatedMock?.owners);
        repository.getCount.mockResolvedValue(ownerEntityPaginatedMock?.total);
    ...
    test("should call add of addOwner with correct values", async () => {
        await testInstance.addOwner(ownerEntityMock);
        expect(repository.add).toHaveBeenCalledWith(ownerEntityMock);
        expect(repository.add).toHaveBeenCalledTimes(1);
    });
    test("should return a new owner created when addOwner insert it", async () => {
        const result = await testInstance.addOwner(ownerEntityMock);
        expect(result).toEqual(ownerEntityMock);
    });
    ...
    test("should rethrow if add of addOwner throws", async () => {
        ...
        const result = testInstance.addOwner(ownerEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should rethrow if update of updateOwner throws", async () => {
        repository.update.mockRejectedValueOnce(new Error("Error"));
        const result = testInstance.updateOwner(fakeQuery, ownerEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should call update of updateOwner with correct values", async () => {
        await testInstance.updateOwner(fakeQuery, ownerEntityMock);
        expect(repository.update).toHaveBeenCalledWith(fakeQuery?.fields, ownerEntityMock);
        expect(repository.update).toHaveBeenCalledTimes(1);
    });
    test("should return a owner updated when updateOwner update it", async () => {
        const result = await testInstance.updateOwner(fakeQuery, ownerEntityMock);
        expect(result).toEqual(ownerEntityMock);
    });
    test("should return a owner updated when updateOwner update it when i pass null", async () => {
        const result = await testInstance.updateOwner(null as any, ownerEntityMock);
        expect(result).toEqual(ownerEntityMock);
    });
    ...
    test("should return a ownerByPage when loadOwnerByPage loaded it", async () => {
        ...
        expect(result).toEqual(ownerEntityPaginatedMock);
    });
