...
describe("Fidelity Mongo Repository", () => {
    ...
    beforeAll(async () => {
        ...
        repository.add.mockResolvedValue(fidelityEntityMock);
        repository.getOne.mockResolvedValue(fidelityEntityMock);
        repository.update.mockResolvedValue(fidelityEntityMock);
        repository.getPaginate.mockResolvedValue(fidelityEntityPaginatedMock?.fidelities);
        repository.getCount.mockResolvedValue(fidelityEntityPaginatedMock?.total);
    ...
    test("should call add of addFidelity with correct values", async () => {
        await testInstance.addFidelity(fidelityEntityMock);
        expect(repository.add).toHaveBeenCalledWith(fidelityEntityMock);
        expect(repository.add).toHaveBeenCalledTimes(1);
    });
    test("should return a new fidelity created when addFidelity insert it", async () => {
        const result = await testInstance.addFidelity(fidelityEntityMock);
        expect(result).toEqual(fidelityEntityMock);
    });
    ...
    test("should rethrow if add of addFidelity throws", async () => {
        ...
        const result = testInstance.addFidelity(fidelityEntityMock);
    ...
    test("should rethrow if update of updateFidelity throws", async () => {
        ...
        const result = testInstance.updateFidelity(fakeQuery, fidelityEntityMock);
        await expect(result).rejects.toThrow("Error");
    });
    test("should call update of updateFidelity with correct values", async () => {
        await testInstance.updateFidelity(fakeQuery, fidelityEntityMock);
        expect(repository.update).toHaveBeenCalledWith(fakeQuery?.fields, fidelityEntityMock);
        expect(repository.update).toHaveBeenCalledTimes(1);
    });
    test("should return a fidelity updated when updateFidelity update it", async () => {
        const result = await testInstance.updateFidelity(fakeQuery, fidelityEntityMock);
        expect(result).toEqual(fidelityEntityMock);
    });
    test("should return a fidelity updated when updateFidelity update it when i pass null", async () => {
        const result = await testInstance.updateFidelity(null as any, fidelityEntityMock);
        expect(result).toEqual(fidelityEntityMock);
    });
    ...
    test("should return a fidelityByPage when loadFidelityByPage loaded it", async () => {
        ...
        expect(result).toEqual(fidelityEntityPaginatedMock);
    });
