...
import { CustomerData, CustomerPaginatedData } from "@/slices/customer/entities";
import {
        ...
        UpdateCustomerRepository
{
    ...
    async incrementAppointmentsTotal(query: Query): Promise<CustomerData | null> {
        return this.repository.increment(query?.fields ?? {}, { appointmentsTotal: 1 });
    }
    async addCustomer(client: CustomerData): Promise<CustomerData | null> {
        return this.repository.add(client);
    }
    async deleteCustomer(query: Query): Promise<CustomerData | null> {
        return this.repository.deleteOne(query?.fields);
    }
    async loadCustomerByPage(query: Query): Promise<CustomerPaginatedData | null> {
